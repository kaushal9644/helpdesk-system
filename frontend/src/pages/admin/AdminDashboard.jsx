...
function AdminDashboard() {
  ...
  const updateProfile = async () => {
    if (profilePassword || profileConfirmPassword) {
      ...
      const confirmChange = window.confirm("Do you want to change password?");
  ...
  const loadComments = async (ticketId) => {
    try {
      ...
      setComments((prev) => ({ ...prev, [ticketId]: response.data.data || [] }));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const deleteTicket = async (ticketId) => {
    ...
    const confirmDelete = window.confirm("Are you sure? Only resolved tickets can be deleted.");
  ...
  const deleteResolvedOlderThan = async (days) => {
    ...
    const confirmDelete = window.confirm(`Delete all resolved tickets older than ${days} days?`);
    if (!confirmDelete) return;
  ...
  const addComment = async (ticketId) => {
    ...
    try {
      await axios.post(
        ...
        { ticketId, comment: newComment[ticketId] },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const filteredTickets = tickets.filter((ticket) => {
    ...
    const branchMatch = !branchFilter || ticket.branch?.branchName === branchFilter;
  ...
  const branchChartData = branchStatsArray.map(([branch, count]) => ({ branch, count }));

  // ── Reusable Employees Panel (used in both desktop two-col and mobile employees tab) ──
      ...
      {!showAddForm ? (
        ...
        <div className="adm-add-form">
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button className="adm-add-btn" onClick={createEmployee} disabled={createLoading}>
              {createLoading ? "Creating..." : "Submit"}
            </button>
            <button
              className="adm-attach-btn"
              style={{ padding: '8px 18px', fontSize: 12 }}
              onClick={() => { setShowAddForm(false); setName(""); setEmail(""); setPassword(""); }}
            >
              Cancel
            </button>
          </div>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <span
                  style={{
                    ...
                    fontSize: "10px",
                    fontWeight: "600",
                    background: employee.role === "ADMIN" ? "rgba(74,222,128,0.12)" : "rgba(96,165,250,0.12)",
                    color: employee.role === "ADMIN" ? "#4ade80" : "#60a5fa",
                    border: employee.role === "ADMIN" ? "1px solid rgba(74,222,128,0.25)" : "1px solid rgba(96,165,250,0.25)",
                  }}
                >
                  {employee.role}
                </span>
          ...
          (Array.isArray(filteredTickets) && filteredTickets.length > 0 ? filteredTickets : tickets).map((ticket) => {
            ...
            return (
              <div key={ticket.id} className="adm-ticket-card">
                ...
                {/* Description */}
                {ticket.description && <p className="adm-ticket-desc">{ticket.description}</p>}
  ...
  return (
    <>
      ...
      <div className="adm-root" style={{ display: "flex", overflow: "hidden" }}>
        ...
        <aside className="adm-sidebar">
          ...
          <nav className="adm-nav">
            ...
            <div
              ...
              className={`adm-nav-item ${activeTab === "tickets" ? "active" : ""}`}
            >
              <Ticket size={15} color="#94a3b8" />
              <span>Tickets</span>
              <span className="adm-badge gray">{tickets.length}</span>
            </div>

            <div
              onClick={() => setActiveTab("employees")}
              className={`adm-nav-item ${activeTab === "employees" ? "active" : ""}`}
            >
              <Users size={15} color="#94a3b8" />
              <span>Employees</span>
              <span className="adm-badge gray">{employees.length}</span>
            </div>
        ...
        {/* Mobile Bottom Nav — Dashboard / Tickets / Employees */}
        <nav className="adm-mobile-nav">
          <button
            className={`adm-mobile-nav-item ${activeTab === "dashboard" ? "active" : ""}`}
            onClick={() => setActiveTab("dashboard")}
          >
          ...
          <button
            className={`adm-mobile-nav-item ${activeTab === "tickets" ? "active" : ""}`}
            onClick={() => setActiveTab("tickets")}
          >
            <Ticket size={20} />
            <span>Tickets</span>
            {tickets.length > 0 && <span className="badge-count">{tickets.length}</span>}
          </button>

          <button
            className={`adm-mobile-nav-item ${activeTab === "employees" ? "active" : ""}`}
            onClick={() => setActiveTab("employees")}
          >
            <Users size={20} />
            <span>Employees</span>
            {employees.length > 0 && <span className="badge-count">{employees.length}</span>}
          </button>
        </nav>
        ...
        <main style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
          ...
          <header className="adm-topbar">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <button onClick={() => setProfileOpen(true)} className="adm-profile-btn">
                <User size={14} />
                Profile
              </button>
              ...
              {/* Logout - visible only on mobile (sidebar hidden there) */}
              <button
                className="adm-btn-danger adm-mobile-logout"
                onClick={() => { localStorage.clear(); window.location.href = "/"; }}
                style={{ padding: '7px 14px', fontSize: 11, gap: 4 }}
              >
                <LogOut size={16} />
                Logout
              </button>
          ...
          <div style={{ padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 1600, width: '100%', margin: '0 auto' }}>
            ...
            {activeTab === "tickets" && (
              <>
                ...
                {/* Two col on desktop, single column on mobile (Tickets only here, Employees in its own tab) */}
                <div className="adm-two-col">
                  {TicketsPanel}
                  {/* Employees panel still shown alongside on desktop */}
            ...
            {activeTab === "employees" && (
              <div className="adm-mobile-only-tab">
                {EmployeesPanel}
              </div>
            )}
      ...
      {profileOpen && (
        <div className="adm-modal-overlay">
          <div className="adm-profile-modal">
            <button className="adm-modal-close" onClick={() => setProfileOpen(false)}>
              <X size={14} />
            </button>

            <h2 className="adm-profile-title">My Profile</h2>

            <p className="adm-profile-email">{profileData?.email}</p>
            ...
            <button className="adm-add-btn" onClick={updateProfile} disabled={profileLoading}>
      ...
      {selectedTicket && (
        <div className="adm-modal-overlay">
          <div className="adm-profile-modal" style={{ width: "620px" }}>
            ...
            <h2 className="adm-profile-title">{selectedTicket.title}</h2>
            ...
            <div className="adm-modal-grid">
              <div className="adm-form-input">Status: {selectedTicket.status}</div>
              <div className="adm-form-input">Branch: {selectedTicket.branch?.branchName || "N/A"}</div>
            </div>

            <h3 style={{ fontSize: 13, color: "#f1f5f9", marginTop: 10 }}>Attachments</h3>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {selectedTicket.attachments?.length > 0 ? (
                ...
                <p style={{ color: "#64748b", fontSize: 12 }}>No attachments</p>
              )}
            </div>

            <h3 style={{ fontSize: 13, color: "#f1f5f9", marginTop: 10 }}>Comments</h3>
